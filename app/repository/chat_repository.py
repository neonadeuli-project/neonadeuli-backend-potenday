...
class ChatRepository:
    ...
    async def create_chat_session(
        ...
        logger.info(
            f"ChatRepository에서 채팅 세션을 생성합니다. (user_id: {user_id}, heritage_id: {heritage_id})"
        )
        try:
            # 채팅 세션 활성화 상태 확인
            ...
            if active_session:
                logger.info(
                    f"기존 활성 세션을 반환합니다. (session_id: {active_session.id})"
                )
            ...
            if not user:
                raise UserNotFoundException(
                    f"등록되지 않은 사용자입니다. (user_id: {user.id})"
                )

            # 문화재 여부 확인
            heritage = await self.db.execute(
                select(Heritage).where(Heritage.id == heritage_id)
            )
            heritage = heritage.scalar_one_or_none()
            if not heritage:
                raise HeritageNotFoundException(
                    f"등록되지 않은 문화재입니다. (heritage_id: {heritage_id})"
                )
            ...
            logger.info(
                f"새로운 채팅 세션이 생성되었습니다. (session_id: {new_session.id}, user_id: {user_id}, heritage_id: {heritage_id})"
            )
            ...
            logger.error(
                f"채팅 세션 생성 중 에러 발생: {str(e)}", exc_info=True
            )
            raise
        except SQLAlchemyError as e:
            logger.error(
                f"Database error in create_chat_session: {str(e)}",
                exc_info=True,
            )
            raise DatabaseOperationException(
                "채팅 세션 생성 중 데이터베이스 오류 발생"
            )
        except Exception as e:
            logger.error(
                f"Unexpected error in create_chat_session: {str(e)}",
                exc_info=True,
            )
            raise ChatServiceException(
                "채팅 세션 생성 중 예상치 못한 오류 발생"
            )
    ...
    async def get_active_session(
        ...
        try:
            ...
            logger.error(
                f"채팅 활성화 상태 조회 중 데이터베이스 오류 발생 : {str(e)}",
                exc_info=True,
            )
            raise DatabaseOperationException(
                "활성 세션 조회 중 데이터베이스 오류 발생"
            )
    ...
    async def end_chat_session(self, session_id: int) -> Optional[ChatSession]:
        try:
            ...
            logger.error(
                f"채팅 세션 ID가 {session_id} 인 채팅 세션이 종료되는 동안 데이터 베이스에 오류가 발생했습니다.: {str(e)}"
            )
            ...
            logger.error(
                f"채팅 세션 ID가 {session_id} 인 채팅 세션이 종료되는 동안 예상치 못한 에러 발생 : {str(e)}"
            )
            await self.db.rollback()
    ...
    async def create_message(
        self, session_id: int, role: RoleType, content: str
    ) -> ChatMessage:
        try:
            ...
            logger.error(
                f"메시지 생성 중 데이터베이스 오류 발생: {str(e)}",
                exc_info=True,
            )
            raise DatabaseOperationException(
                "메시지 생성 중 데이터베이스 오류 발생"
            )
    ...
    async def update_message(self, session_id: int, **kwargs):
        try:
            ...
            logger.error(
                f"메시지 업데이트 중 데이터베이스 오류 발생: {str(e)}",
                exc_info=True,
            )
            await self.db.rollback()
            raise DatabaseOperationException(
                "메시지 업데이트 중 데이터베이스 오류 발생"
            )

    # 채팅 최근 저장된 메시지 1개 조회
    async def get_latest_message(
        self, session_id: int, role: RoleType
        ...
        try:
            ...
            logger.error(
                f"최근 메시지 조회 중 데이터베이스 오류 발생: {str(e)}",
                exc_info=True,
            )
            await self.db.rollback()
            raise DatabaseOperationException(
                "최근 메시지 조회 중 데이터베이스 오류 발생"
            )
    ...
    async def get_chat_session(self, session_id: int) -> Optional[ChatSession]:
        try:
            ...
            logger.error(
                f"채팅 세션 조회 중 데이터베이스 오류 발생: {str(e)}",
                exc_info=True,
            )
            raise DatabaseOperationException(
                "채팅 세션 조회 중 데이터베이스 오류 발생"
            )
    ...
    async def save_recommended_questions(
        self, session_id: int, questions: List[str]
    ):
                ...
                try:
                    ...
                    await session.execute(
                        delete(RecommendedQuestion).where(
                            RecommendedQuestion.session_id == session_id
                        )
                    )
                    # 새로운 추천 질문 저장
    ...
    async def get_chat_summary(self, session_id: int):
        try:
            ...
            if (
                chat_session
                and chat_session.summary_keywords
                and chat_session.visited_buildings
            ):
                return {
            ...
            logger.error(
                f"채팅 요약 조회 중 데이터베이스 오류 발생: {str(e)}",
                exc_info=True,
            )
            raise DatabaseOperationException(
                "채팅 요약 조회 중 데이터베이스 오류 발생"
            )
    ...
    async def save_chat_summary(
        ...
        session_id: int,
        keywords: List[str],
        visited_buildings: List[VisitedBuilding],
    ):
        try:
            await self.db.execute(
                update(ChatSession)
                ...
                .values(
                    ...
                    visited_buildings=[
                        building.model_dump() for building in visited_buildings
                    ],
            ...
            logger.error(
                f"채팅 요약 저장 중 데이터베이스 오류 발생: {str(e)}",
                exc_info=True,
            )
            await self.db.rollback()
            raise DatabaseOperationException(
                "채팅 요약 저장 중 데이터베이스 오류 발생"
            )
