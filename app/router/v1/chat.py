...
async def create_chat_session(
    chat_session: ChatSessionCreateRequest, db: AsyncSession = Depends(get_db)
):
    chat_service = ChatService(db)
    try:
        return await chat_service.create_chat_session(
            chat_session.user_id, chat_session.heritage_id
        )
        ...
        logger.error(
            f"채팅 세션 생성 중 예상치 못한 오류 발생: {str(e)}", exc_info=True
        )
        raise HTTPException(
...
async def add_chat_message(
    ...
    chat_service = ChatService(db)
    try:
        return await chat_service.update_chat_conversation(
        ...
        logger.error(
            f"메시지 전송 중 예상치 못한 오류 발생: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
# 건축물 정보 제공
@router.post(
    "/{session_id}/heritage/buildings/info",
    response_model=BuildingInfoButtonResponse,
)
async def get_heritage_building_info(
    ...
    chat_service = ChatService(db)
    try:
        return await chat_service.update_info_conversation(
            session_id, building_data.building_id
        )
        ...
        logger.error(
            f"건축물 정보 제공 중 예상치 못한 오류 발생: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
# 건축물 퀴즈 제공
@router.post(
    "/{session_id}/heritage/buildings/quiz",
    response_model=BuildingQuizButtonResponse,
)
async def get_heritage_building_quiz(
    ...
    chat_service = ChatService(db)
    try:
        return await chat_service.update_quiz_conversation(
            session_id, building_data.building_id
        )
        ...
        logger.error(
            f"퀴즈 제공 중 예상치 못한 오류 발생: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="서버 오류가 발생했습니다.",
...
# 건축물 추천 질문 제공
@router.post(
    "/{session_id}/building/recommend-questions",
    response_model=RecommendedQuestionResponse,
)
async def get_building_recommented_questions(
    ...
    chat_service = ChatService(db)
    try:
        return await chat_service.get_building_questions(
            session_id, building_data.building_id
        )
...
# 메시지 추천 질문 제공
@router.get(
    "/{session_id}/message/recommend-questions", response_model=List[str]
)
...
# 채팅 요약 제공
@router.get(
    "/sessions/{session_id}/summary", response_model=ChatSummaryResponse
)
...
# 채팅 세션 종료
@router.post(
    "/sessions/{session_id}/end", response_model=ChatSessionEndResponse
)
...
# 채팅 세션 종료 여부 확인
@router.get(
    "/sessions/{session_id}/status", response_model=ChatSessionStatusResponse
)
async def check_chat_session_status(
    session_id: int, db: AsyncSession = Depends(get_db)
):
    chat_service = ChatService(db)
    try:
        ...
        return ChatSessionStatusResponse(
            session_id=session_id, ended_status=ended_status
        )
    except SessionNotFoundException as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=str(e)
