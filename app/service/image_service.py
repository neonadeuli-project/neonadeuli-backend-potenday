...
class ImageService:
    ...
    async def add_building_image(
        ...
        try:
            # 건축물과 문화재 유효성 검증
            belongs_to_heritage = await self.heritage_repository.verify_building_belongs_to_heritage(heritage_id, building_id)
            if not belongs_to_heritage:
                raise InvalidImageFormatException(f"건축물 ID {building_id}번인 건축물은 문화재 ID {heritage_id}번인 문화재에 존재하지 않습니다.")
            
            # 이미지 저장
            result = await self.image_repository.add_building_image(heritage_id, building_id, image_url, description, alt_text)
            return result
