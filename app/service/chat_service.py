...
class ChatService:
    ...
    async def end_chat_session(self, session_id: int) -> ChatSessionEndResponse:
        ...
        try:
            ...
            return ChatSessionEndResponse(session_id=ended_session.id, end_time=ended_session.end_time)
    ...
    async def update_info_conversation(self, session_id: int, building_id: int) -> BuildingInfoButtonResponse:
        try:
            ...
            return BuildingInfoButtonResponse(image_url=image_url or "", bot_response=bot_response or "")
        except (
            SessionNotFoundException,
    ...
    async def update_quiz_conversation(self, session_id: int, building_id: int) -> BuildingQuizButtonResponse:
        try:
            chat_session, building = await self.validation_service.validate_session_and_building(
    ...
    async def get_building_questions(self, session_id: int, building_id: int) -> RecommendedQuestionResponse:
        try:
            ...
            # 최대 3개 질문만 사용
            return RecommendedQuestionResponse(building_id=building_id, questions=questions[:3])
    ...
    async def generated_and_save_chat_summary(self, session_id: int, visited_buildings: List[VisitedBuilding]):
        try:
            ...
            # 생성된 키워드 DB 저장
            await self.chat_repository.save_chat_summary(session_id, summary_response["keywords"], visited_buildings)
